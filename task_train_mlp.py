...
model = MLP(2,3*32*32,10,0.2).to(device)
...
for epoch in range(save_epoch):
    ...
    for data, target in train_loader:
        ...
        optimizer.step()
    acc, test_loss, _ = evaluate_model(model, test_loader, device)
    print(f'In Save Epoch:{epoch}/{save_epoch}, Test Acc is:{acc}, Test Loss is:{test_loss}')
    model.eval()
...
torch.save(save_info, './param_data/mlp-cifar10.pt')
